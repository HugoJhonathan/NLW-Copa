...
export async function guessRoutes(fastify: FastifyInstance) {
    ...
    fastify.post('/pools/:poolId/games/:gameId/guesses', { onRequest: [authenticate] }, async (request, reply) => {
        ...
        // nao permitir criar dois palpite no mesmo bolao
        ...
        if (guess) {
            return reply.status(400).send({
                message: "You alread sent a guess to this game on this pool."
            })
        }
        ...
        let guessCreated = await prisma.guess.create({
            data: {
                gameId,
                participantId: participant.id,
                firstTeamPoints,
        ...
        return reply.status(201).send(guessCreated)
